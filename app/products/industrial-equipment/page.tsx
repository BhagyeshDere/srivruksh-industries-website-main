...
import { ArrowRight, Shield, Settings2, Zap, Cpu } from "lucide-react"
...
export default function IndustrialEquipmentPage() {
  ...
  // Filter specifically for the Industrial Equipment category
  const filteredProducts = products.filter(
    (p) => p.category === "Industrial Equipment"
  )
    ...
    <main className="bg-[#f8fafc] min-h-screen overflow-x-hidden">
      ...
      <section className="relative bg-[#0f172a] py-16 sm:py-20 md:py-28 lg:py-32 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto flex flex-col items-center px-2 text-center">
          ...
          >
            <Zap size={14} className="text-[#C79A3B]" />
          ...
          >
            Next-generation manufacturing and structural assets engineered to 
            withstand the rigors of high-intensity industrial environments.
          </motion.p>
        </div>
      </section>
              ...
              {filteredProducts.map((item, index) => (
                ...
                >
                  ...
                  <div className="absolute top-6 left-8 right-8 flex justify-between items-center z-10">
                    ...
                    <Cpu size={16} className="text-slate-200 group-hover:text-[#C79A3B] transition-colors" />
                  </div>
                  ...
                  <div className="p-8 sm:p-10 bg-gradient-to-t from-slate-50 to-transparent relative border-t border-slate-50 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-slate-100/50">
                      <a
                        href={`/products/industrial-equipment/${item.slug}`}
                      ...
                      <div className="flex flex-col items-end opacity-20 group-hover:opacity-40">
                        ...
                        <span className="text-[7px] font-bold uppercase mt-1">ISO 9001</span>
                      </div>
                    </div>
                  </div>

                  {/* Visual Accent */}
                  <div className="absolute top-0 right-0 w-24 h-24 bg-[#C79A3B]/5 -mr-12 -mt-12 rotate-45 group-hover:bg-[#C79A3B]/10 transition-colors" />
                </motion.div>
              ))}
            </AnimatePresence>
          </div>
